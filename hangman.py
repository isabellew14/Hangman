...
from string import asciss_uppercase
import random

window=Tk()
window.title("Hangman Game")

list_words = ["RUSSIA","ARGENTINA", "MEXICO","COLOMBIA","KAZAKHSTAN","GERMANY","TURKEY","HUNGARY",
              "FINLAND","INDIA","CHINA","SWEDEN","MONGOLIA","MOROCCO","NORWAY","CHILE","IRELAND",
              "ICELAND","CROATIA","ESTONIA","SPAIN","NIGERIA","JAPAN","UKRAINE","ALGERIA"]
...
def guess(letter):
    ...
    if the_word_withpaces.count(letter)>0:
        for c in range(len(txt)):
                if txt[c]==letter:
                    guessed[c]=letter
                lblWord.set("".join(guessed))
                if lblWord.get()==the_word_withSpaces:
                    messagebox.showinfo("Hangman:","You guesssed it!")
    else:
        numberOfGuesses+=1
        imgLabl.config(image=photos[numberOfGuesses])
        if numberOfGuesses==11:
...
imgLabel.grid(row=0,column=0,columspan=3, padx=10, pady=40)
imgLabel.config(image=photos[0])

lblWord=StringVar()
Label(window, textvariable=lblWord, font=("Consolas 24 bold")).grid(row=0,column=3,columspan=6, padx=10)
